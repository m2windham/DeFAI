...
|---|---|---|
| 25 — decorrelation α-sweep | Is embedding correlation the whole cause of the pool-mode perception collapse (85/395 at scale)? Sweep the PPMI-SVD weighting `emb = U·S^α`, α ∈ {0.5, 0.25, 0}; report the >0.7-neighbor fraction, transitive-component count, and core-arm coverage per α. Pre-registered: if correlation is the whole story, α=0 collapses the neighbor fraction and coverage recovers toward the recipe arm's 378/395. Neighbor stat collapsing *without* coverage recovering = the bars themselves are a second cause (feeds Phase 26 either way). | `phase25_decorrelation_sweep.py` |
